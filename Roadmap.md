...
| — | **54 live-fire fixes P1–P4** (default template, no-scoreboard health check, OCR progress, consolidated game page) | Done (Pass 23) | Live-fire against a real 2.4h game: unconfigured runs silently produced zero results (P1/P2), the 20–40 min OCR phase looked frozen (P3), and managing a game required hopping across three pages (P4). See item 54 section for details. |
...
| — | **54** — Turnkey Web App (epic) | Done (54a/b/c/e + 54d, Passes 23–27) | **Release gate.** Make the web app usable by a non-technical coach: auto-provision ffmpeg via pip, one-command launch that opens the browser, in-app onboarding, and (endgame) a double-clickable bundled app. Phases 54a–54e. Motivated by live-fire prep — the owner couldn't start it unaided. |
...
| 9b | **52** — Persist Roster Display Name | Done (Pass 25) | Small. Roster CSV doesn't store the pretty team name → reloads as the file stem (e.g. `st_mary_s_12u`). Persist via a `# team_name:` header line, stem fallback. Surfaced by item 50 review. |
...
| — | **19** — Full Windows Support | Done (Pass 31) | Cross-platform README/checklist, portable `next_commands`, `requires-python`>=3.10, and a 3-OS GitHub Actions CI matrix (pytest). Code was already pathlib-clean; this closed the docs/CI gaps. |
...
Status: Ready for review (`impl/item-65`)
