...
#### CR-61 — OCR numeric confidence regression test's absolute-accuracy threshold is Tesseract-version-sensitive
**File:** [tests/test_ocr.py](tests/test_ocr.py) — `NumericConfidenceRegressionTests.test_shipped_numeric_pipeline_does_not_regress`
**Pass:** 35 (Reported by Fable 5 during item 65; triaged) — **Status: Deferred (environmental)**

Fable filed this while verifying item 65: the full suite failed only this test — `new_correct` was `4`, below the `len(CASES) - 1 == 5` absolute threshold. **Independently confirmed this pass:** the single test fails identically on both `impl/item-65` and untouched `main` (`4 not >= 5`), so it is unrelated to item 65's web-server-lifecycle changes. The test's *relative* guard (`new_correct >= old_correct`) still passes — the new numeric pipeline is not a regression against the old one; only the absolute-count assertion trips. This assertion does live Tesseract OCR on synthetic digit crops, so its pass/fail is sensitive to the local Tesseract build and interpreter (here: local Tesseract + Python 3.9.6; the project targets 3.10+ and the CI matrix runs 3.10/3.14). Deferred rather than Open because it is an environment/test-robustness issue, not a product defect: the durable fix is to make the assertion robust to Tesseract-version accuracy jitter (compare against the old pipeline / a per-environment baseline rather than a hard `len(CASES)-1`). **Promote to Open if the 3-OS CI matrix goes red on this test** — that would mean it fails on a supported interpreter, not just the local 3.9 box.

#### CR-60 — Overlay-advanced trailing half-inning emits a chapter after the game is decided (needs a SidelineHD signal we don't have)
**File:** [events.py](src/sidelinehd_extractor/events.py) — half-inning chapter emission near `GAME_FINAL`
**Pass:** — (observed alongside CR-59 on `Zv469PLcc7o`)
**Status:** Deferred — **product owner's call: valid, but no reliable signal yet**

In the run-rule-shortened `Zv469PLcc7o` game (final 11-0 after 3 innings) SidelineHD briefly advanced the overlay to **Top 4** before FINAL — frame-verified `▲4` at 3730s with one detected at-bat — so the pipeline faithfully emitted a `Top 4` chapter the videographer considers post-game. **Ryan's disposition (2026-07-10): expected to recur whenever games end early; deferred, not fixed now.** GameChanger surfaces an explicit "Inning ended by scorekeeper" status in its feed, but **SidelineHD exposes no equivalent signal**, so we cannot cleanly detect "the scorekeeper ended the inning." Two candidate heuristics both have holes:
- *"No batters from our team in the half"* — fails for our **defensive** half-innings, which legitimately show only the pitcher card and a score with no batter events.
- *"Score didn't change"* — insufficient: a real defensive half can be scoreless, and we don't capture whether any pitches were actually thrown, so a static score is not proof the half was empty/aborted.
A future improvement is desirable but needs a trustworthy "half actually played" indicator we don't yet have. Revisit if SidelineHD adds a scorekeeper-status field or if pitch/activity capture lands.
...
**File:** [cli.py](src/sidelinehd_extractor/cli.py), [webapp/lifecycle.py](src/sidelinehd_extractor/webapp/lifecycle.py) (new), [webapp/app.py](src/sidelinehd_extractor/webapp/app.py), [webapp/templates/_footer.html](src/sidelinehd_extractor/webapp/templates/_footer.html) (new, included on all full pages), [README.md](README.md), [NEW_GAME_CHECKLIST.md](NEW_GAME_CHECKLIST.md); [tests/test_webapp_lifecycle.py](tests/test_webapp_lifecycle.py) (new), [tests/test_webapp.py](tests/test_webapp.py)
**Pass:** 35 (by Fable 5, branch `impl/item-65`)
**Resolved:** Pass 35

Implements all three scope items and every acceptance criterion of the item 65 design. Verified against the design and independently re-run:

- **State file + `stop`/`status`/`restart`.** `serve`/`start` write `webapp.json` under `desktop.default_data_dir()` via a `ServerStateRegistration` context manager (`atexit` **and** a SIGTERM handler for cleanup), written *before* `uvicorn.run` so the app can read it at startup. `stop` reads the record, SIGTERMs a live PID, polls ~5s, then escalates — correctly gated `signal.SIGKILL if hasattr(signal, "SIGKILL") else SIGTERM` for Windows — and clears the file; a dead/stale PID (`os.kill(pid, 0)` raises) is detected and cleared by both `status` and `stop` without error; `restart` runs the stop path (tolerant of "not running") then hands off to a fresh foreground `start`. `status` always exits 0.
- **Version+start banner + footer.** Banner shows `v<version>` plus best-effort `git rev-parse --short HEAD` (swallowed on failure/wheel installs). New `_footer.html`, included on all full-page templates, renders `v<version> · started <time>` via `footer_runtime_label()` — the signal that would have made the 2026-07-10 stale-server incident self-evident. Footer string is computed once at `create_app` (correct: a server process has a fixed start time).
- **Smarter port-in-use.** When the busy port belongs to *our own* recorded server, `start`/`serve`/`restart` print the targeted "already running — use `restart`/`stop`" hint; an unrecorded busy port keeps the generic "pick another `--port`" message.

**Independent verification:** full suite **549 passed, 1 failed** (only the pre-existing OCR test → CR-61, confirmed identical on `main`), ruff clean. New tests patch `uvicorn.run` (no real server bound) and redirect `default_data_dir` to `tmp_path` (no real data-dir pollution); coverage spans state round-trip, live-PID stop, SIGKILL escalation, stale-record clearing, status formatting, both port-in-use branches, restart stop-then-start, and footer rendering. **No new dependency.**

**Deviations (both local-tier, flagged and accepted):** (1) state/PID logic factored into a focused `webapp/lifecycle.py` rather than inlined in `cli.py` — cleaner, and gives the web UI a direct footer helper (the design explicitly permitted this); (2) footer included on *all* full pages, not only results/review — the design's "already carry a footer credit line" premise was inaccurate (no prior `<footer>` existed), so a new one visible everywhere is the right call, no duplication. **Minor note (non-blocking):** `footer_runtime_label()` reads the shared `webapp.json`, which only the CLI writes — if a CLI server were hard-killed (SIGKILL, no cleanup) leaving a stale record and the desktop menubar app were then launched, its footer could show the dead CLI server's start time; rare, and still reads as a "this looks old" signal. Revisit only if it bites.

#### CR-59 — GAME_FINAL exports no score when the FINAL banner blanks the scorebug
**File:** [events.py](src/sidelinehd_extractor/events.py) — `_detect_game_final`, new `_last_complete_score_before`; [tests/test_events.py](tests/test_events.py)
**Pass:** — (user live-fire report, 2026-07-10, video `Zv469PLcc7o`)
**Resolved:** Pass 34 — **role deviation: implemented by the architect at Ryan's direct instruction**

**Live-fire evidence (`Zv469PLcc7o`, 640×360 native):** the Final chapter exported with **no score** (should be `(11-0)`). `_detect_game_final` derived the final score from `_score_snapshot(states, run_start_index, run_length)` — a window spanning **only the FINAL-banner run**. When SidelineHD swaps in the FINAL banner it blanks the live scorebug, so `home_score` reads `None` throughout that window and no complete `(away, home)` pair exists → `(None, None)`. The true score read solidly *just before* the banner (frame-verified `away=11, home=0`, 3730–3755s). The last half-inning chapter also inherited the `None` (it reads the FINAL as its "next" score).

**Fix.** New `_last_complete_score_before(states, index)` scans backward from the FINAL-run start for the nearest complete `(away, home)` pair; `_detect_game_final` uses it as a fallback whenever the in-banner snapshot is missing either score. Scores are cumulative and the game has just ended, so the nearest complete pre-banner pair *is* the final score — no risk of grabbing a mid-inning value. Regression test `test_detect_game_final_recovers_score_when_banner_blanks_scorebug` seeds a FINAL run of blanked (`home=None`) states preceded by an `11/0` pair and asserts the GAME_FINAL carries `(11, 0)` (fails without the fix, verified).

**End-to-end re-validation:** re-ran detect+export on the real `Zv469PLcc7o` states — **Final now `(11-0)`** (was blank), and the last half-inning chapter recovers its score too. 540 tests pass (1 new), ruff clean. The CR-60 trailing "Top 4" chapter still appears (deferred by Ryan) but now carries `(11-0)` rather than a blank.
...
**Pass:** 33 (by Fable 5, branch `impl/item-64`)
**Resolved:** Pass 33

Live-fire runs of `8WlQ-exJOkE` emitted a phantom `at_bat_start` for #2 at 1:26:05 (bottom-5 leadoff, `lineup-recovered`) in 3/3 runs; #2's real at-bat is at 1:29:55 and the established order gives the slot to #26. Implementation: a veto at the order-validator seam in `validate_batting_order`. An at-bat is suppressed only when all three hold: (a) its id came solely from lineup-strip recovery (`batter_number_source == lineup_strip`, no name corroboration), (b) it sits at a non-zero forward offset **within the established cycle**, and (c) the order-conforming expected batter (`cycle[cycle_pos]`) still bats later in the same half-inning. Players outside the seed cycle are never vetoed (the `player_number not in cycle` branch handles them before `cycle.index`), and genuine substitutes replace the conforming batter who then never reappears — condition (c) is what distinguishes them from phantoms. Suppression `continue`s without advancing the cycle tracker, so the real conforming batter matches cleanly.

**Independent re-validation (this pass, reproduced not assumed):** replayed the auto-detect detect→infer-half→filter→`validate_batting_order`→export path against **both** real runs (`…-132646` and `…-130437`) with old (pre-CR-58) vs new code. The *only* diffs old→new in either run: the 1:26:05 #2 phantom is removed from events and the at-bats export, and the 1:26:50 #26 at-bat loses its spurious `out-of-order-candidate` flag (it existed only because the phantom desynced the tracker). **Chapters exports byte-identical; every other at-bat line byte-identical.** Verified the substitute path is not over-vetoed by logic + `test_validate_batting_order_keeps_lineup_recovered_substitute`. 3 new tests; 517 (branch) tests pass, ruff clean.

---

#### CR-55 — Just-ended livestreams (`post_live`) fail with a misleading raw yt-dlp error
...
A just-ended stream (`youtube.com/live/<id>`, `live_status=post_live`) fails to download on yt-dlp 2026.7.4 with the raw "ERROR: This live event has ended." shown verbatim (2025.10.14 downloads it fine; `-F` succeeds even when download fails). Implementation: `download_youtube_video` probes **on failure only** — when the download subprocess fails, a cheap metadata probe (`yt-dlp --skip-download --no-warnings --print live_status`, mirroring `list_playlist_videos`) checks `live_status`; if `post_live`/`is_live` the error becomes `LiveStreamNotReadyError` (a `YTDLPError` subclass) carrying wait-about-an-hour guidance plus the installed yt-dlp version and the known-good `2025.10.14` note. If the probe fails or reports any other status, the original `YTDLPError` is raised unchanged. Because the fix sits at the `download_youtube_video` seam and both the CLI `YTDLPError` handler and the web `JobStore` (`error=str(exc)`) already surface `str(exc)`, the CLI, playlist batch, and web job status all show the friendly message with no per-caller changes.

**Independent re-validation (this pass):** confirmed by inspection **zero happy-path cost** — the probe is reached only inside the `completed.returncode != 0` branch — and that a failing probe leaves the original error untouched (`test_download_failure_with_failing_probe_keeps_original_error`). The known-good version note deliberately *records* `2025.10.14` rather than pinning `yt-dlp` (part 2 of the original recommendation): no version cap was added, matching the architect's stated preference to avoid a silently-rotting bound. 6 new mocked-runner tests; ruff clean. (`pyproject.toml` `yt-dlp>=2025.1` left unbounded by design.)

---

#### CR-57 — Score capture hardening: template-match the fixed SidelineHD digit font; reject non-monotonic score reads
**File:** [ocr.py](src/sidelinehd_extractor/ocr.py) (`match_scorebug_digits`, `_scorebug_digit_templates`, both backend hooks), [data/scorebug_digits/](src/sidelinehd_extractor/data/scorebug_digits/) (ten bundled reference glyphs), [state.py](src/sidelinehd_extractor/state.py) (`_smooth_score_sequence` in `smooth_states`), [tests/test_ocr.py](tests/test_ocr.py), [tests/test_state.py](tests/test_state.py)
**Pass:** — (user live-fire follow-up, 2026-07-09; implemented by the prior architect at Ryan's direct instruction, **role deviation**)
**Resolved:** Pass 33 — independently re-reviewed

**Problem (continues CR-56):** after resolution normalization, Tesseract *systematically* misreads this overlay font's "9" as "3" — 0/4 under every PSM (7/8/10/13/6), so item-43-style PSM voting cannot help — and "8" reads at 0.12–0.5 confidence, leaving Top/Bottom 4 chapters blank/wrong and a boundary-stray low read exporting a wrong chapter score (Bottom 5 `(2-7)`).

**Fix 1 — template classification before Tesseract.** The scorebug renders one fixed blocky font and item-60 glyph isolation already yields clean binary glyphs, so this is template matching, not OCR. Ten reference digits bundled as package data; `match_scorebug_digits` segments the isolated glyphs (≤2, digit-height), scores pixel agreement at canonical 48×64 against all ten, and accepts only when every glyph clears agreement ≥0.90 with ≥0.03 margin over the runner-up. On any doubt it returns `None` and the existing Tesseract voting path runs unchanged. Wired into both backends for the three `SCOREBUG_ISOLATED_FIELDS`; inning keeps its arrow wrap.

**Fix 2 — monotonic score smoothing.** `smooth_states` passes `away_score`/`home_score` through `_smooth_score_sequence`: an unconfirmed decrease (or a jump of >4 runs) is replaced with the established score; a decrease confirmed by 3 consecutive non-None reads (operator correction) is accepted, large jumps confirm with 2. `None` gaps stay `None` — the guard never invents a score, only suppresses contradictions like the boundary-stray `"2"` that exported `(2-7)`.

**Independent re-validation (this pass, reproduced not assumed):**
- **Packaging (the flagged risk):** `python -m build --wheel` ships all **10** `scorebug_digits/*.png` in the built wheel (not just editable installs), even though the data dir was untracked at review time — hatchling force-includes filesystem files under the package dir.
- **No regression on a different natively-360p game (`PrITEF1eozM`):** re-processed with the new code and diffed raw scorebug reads against the old run field-by-field — **0 text mismatches** across every sampled window. On digit-bearing crops the classifier fires (`source_detail=scorebug_digit_match`) and returns the correct digit; on blank/banner crops it returns `None` and the **Tesseract fallback fires** (`source_detail=None`), identical to old output. CR-56 normalization is a no-op there (native 360p).
- **Downstream:** `smooth_states`'s new score mutation flows into `detect_events`'s `_score_snapshot` as intended; `None` preserved (unit + integration tests).
- 530 tests pass (16 new), ruff clean. The prior architect's full-game re-run reported all 13 chapters correct with zero manual edits (Top 4 `(9-6)`, Bottom 4 `(9-6)`, Top 5 `(12-6)`, Bottom 5 `(12-7)`).

**Security fix during review:** the prior architect's notes named a real player (`#2`) in two places; sanitized to a placeholder (`Emma B.`) before commit.

---

#### CR-56 — OCR collapses on non-native-resolution video: frames must be normalized to the template's calibrated dimensions
**File:** [crops.py](src/sidelinehd_extractor/crops.py) (`normalize_frame_to_template`, new), [processing.py](src/sidelinehd_extractor/processing.py) (`process_video` sampling loop), [template_probe.py](src/sidelinehd_extractor/template_probe.py) (`score_template`), [tests/test_crops.py](tests/test_crops.py)
**Pass:** — (user live-fire report, 2026-07-09, video `8WlQ-exJOkE`; implemented by the prior architect at Ryan's direct instruction, **role deviation**)
**Resolved:** Pass 33 — independently re-reviewed

**Symptom (game of 2026-07-08):** chapter scores missing for Top/Bottom 4 and wrong for Top/Bottom 5 — `(2-6)`/`(2-7)` instead of `(12-6)`/`(12-7)`. **Root cause:** the numeric glyph-isolation preprocessing (items 60/62) is tuned at 360p and drops leading glyphs / collapses confidence at 2×. This video was `post_live` (see CR-55) so only HLS formats existed and the format selector returned **720p for the first time** — the unusual URL state changed the input resolution and exposed the latent bug (every prior VOD run was 640×360).

**Fix (one seam):** new `crops.normalize_frame_to_template(frame, template)` — when the template records native `video_width`/`video_height` and the frame differs, resize the frame to template dimensions (INTER_AREA shrinking, INTER_CUBIC enlarging) before any cropping. Wired into `process_video`'s sampling loop and `template_probe.score_template`. Templates without recorded dimensions and already-matching frames pass through untouched.

**Independent re-validation (this pass):** confirmed a **strict no-op for 640×360**: the default template records `640×360`, so a native-360p frame hits the dims-match branch and returns the *same frame object* (no cv2 call) — reprocessing a 360p run is byte-identical, verified against `PrITEF1eozM` (0 read changes). 720p frames resize as designed. 6 new tests (pass-through, down/upscale, dimensionless template, None frame, fractional-region content preservation); 530 tests pass, ruff clean.

**Residuals from the original report — now dispositioned:** (a) score-monotonicity guard → shipped as CR-57 Fix 2; (b) `"9"`→`"3"` glyph misread → shipped as CR-57 Fix 1; (c) phantom order-impossible `lineup-recovered` at-bat → shipped as **CR-58**; (d) `corrections.py` supports no score fields → still open, worth adding when corrections are next touched.
...
**Pass:** 31 (surfaced) — fixed by Fable 5, branch `impl/cr-54`
**Resolved:** Pass 32

Item 19's new CI matrix red-flagged three independent classes of pre-existing latent test bugs on its first run (all six jobs), none caused by item 19 — the CI working as designed. Fable fixed all three, **test-only, no production source touched**, and iterated against the live matrix (round 1 red → round 2+3 green). Verified by me: the branch's HEAD CI run is green across all six jobs ([run 28822279880](https://github.com/BryceWillis/softball-vision/actions/runs/28822279880)), and it passes locally on my OpenCV-4 + Tesseract-present box (508 pass, ruff clean).

- **OpenCV 5 `putText` (all platforms):** `_make_numeric_crop` now converts the synthesized background to `uint8` before drawing (OpenCV 5 asserts `CV_8U`) and drops `LINE_AA` for cross-version-deterministic rasterization. A round-1 follow-up double-draws the banner fixture at a 2px offset so `test_isolation_rejects_banner_text_spanning_the_crop` still produces a crop-spanning blob under OpenCV 5's tighter glyphs. **Production verified clean** — the only production `putText` (`calibration.py render_template_guide`) draws on decoded uint8 frames.
- **Failure #2 root cause corrected (good catch).** My CR hypothesized cwd/config leakage; Fable disproved it (tree stays clean; test already isolates cwd) and found the real cause: `test_run_youtube_uses_project_config_defaults` ran `main([...])` without `--ocr`, so the default Tesseract PATH preflight exits `main()` before the mocked `run_youtube_game` on any Tesseract-less host (i.e. CI). Fixed with `--ocr none`; reproduced locally by hiding tesseract from PATH.
- **Windows path separators (windows-only):** assertions now build expectations from `Path` objects (missing-file test via `repr(str(Path(...)))` to match `str(OSError)` repr-quoting), and the batch state fixture serializes with `json.dumps` so Windows `\` paths are escaped. Both test-only; the production JSONL writer already escapes correctly.

Two local-tier deviations (dropping `LINE_AA`; the double-draw banner fixture) are appropriate and were flagged. Closes the item 19 CI loop — the cross-platform matrix is now a green, trustworthy regression gate.
