...
**File:** [events.py](src/sidelinehd_extractor/events.py) — `_detect_game_final` / `_score_snapshot`
**Pass:** — (user live-fire report, 2026-07-10, video `Zv469PLcc7o`; reproduced and root-caused by architect)
**Status:** Open

**Live-fire evidence (`Zv469PLcc7o`, 640×360 native):** the Final chapter exported with **no score** (should be `(11-0)`). `_detect_game_final` derives the final score from `_score_snapshot(states, run_start_index, run_length)` — a window that spans **only the FINAL-banner run** (states where `game_status == "final"`). When SidelineHD swaps in the FINAL banner it blanks/replaces the live scorebug, so `home_score` reads `None` throughout that window and no complete `(away, home)` pair exists → `(None, None)`. But the true score read solidly in the states *immediately before* the banner: frame-verified `away=11, home=0` held from 3730–3755s, then home went `None` at the banner (3760s+). The end-of-half chapter for the last half-inning inherits this `None` too (it reads the FINAL as its "next" score), so the last real chapter can also lose its score.

**Fix:** when the in-banner `_score_snapshot` window yields no complete pair, fall back to the **last complete `(away, home)` pair from the states preceding the FINAL run**. Scores are monotonic and the game has just ended, so the nearest complete pre-banner pair *is* the final score — safe, no risk of grabbing a mid-inning value. Add a regression test seeding a FINAL run whose banner states have `home_score=None` and a preceding `11/0` pair; assert the GAME_FINAL event carries `(11, 0)`.

Design sizing: small (one backward-scan fallback in `_detect_game_final` + one test; no data-model change). Independent of CR-57 (the score *digits* now read correctly — this is purely the snapshot window at the FINAL boundary).

## Reported Items (product-decision)

#### CR-60 — Overlay-advanced trailing half-inning emits a chapter after the game is decided
**File:** [events.py](src/sidelinehd_extractor/events.py) — half-inning chapter emission near `GAME_FINAL`
**Pass:** — (observed alongside CR-59 on `Zv469PLcc7o`; **not an OCR bug** — needs a product call)
**Status:** Reported

In the run-rule-shortened `Zv469PLcc7o` game (final 11-0 after 3 innings) SidelineHD briefly advanced the overlay to **Top 4** before showing FINAL — frame-verified `▲4` at 3730s, and the pipeline correctly read it and emitted a `Top 4` chapter (with one detected at-bat) that the videographer considers post-game. This is faithful to what the overlay displayed, so it is a judgment call rather than a defect: should a trailing half-inning that (a) is immediately followed by `GAME_FINAL` and (b) has no scoring advance be suppressed as a chapter? Flagging for Ryan to decide before any change — suppressing real overlay-observed halves risks dropping legitimate short final frames in other games.
