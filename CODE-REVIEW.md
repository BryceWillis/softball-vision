...
#### CR-55 — Just-ended livestreams (`post_live`) fail with a misleading raw yt-dlp error; newer yt-dlp releases make the failure environment-dependent
**File:** [youtube.py](src/sidelinehd_extractor/youtube.py) (`download_youtube_video`, `YTDLPError`), [cli.py](src/sidelinehd_extractor/cli.py#L1410) (`YTDLPError` handler), [pyproject.toml](pyproject.toml) (`yt-dlp>=2025.1` unbounded)
**Pass:** — (user live-fire report, 2026-07-08; reproduced and verified by architect)
**Status:** Open

**Live-fire evidence (video `8WlQ-exJOkE`, submitted a few hours after the stream ended, `live_status: post_live`):**
- With **yt-dlp 2026.7.4** (what a fresh `pip install` gets today), the download fails on **every** player client (android/web/tv/ios/mweb) with `ERROR: This live event has ended.` plus SABR-experiment / PO-token warnings. `run-youtube` surfaces that raw stderr verbatim — a coach reads "this live event has ended" as a fatal video problem when the truth is "YouTube hasn't finished processing the VOD yet; also your downloader version has a regression."
- With **yt-dlp 2025.10.14**, the identical command downloads the same `post_live` video fine (720p HLS formats 91–95; the eventual 1080p progressive VOD doesn't exist yet). So the failure is a *downloader-version* interaction, not an inherent YouTube block — the worst kind of turnkey landmine: works on the developer's machine, fails on a fresh install.
- Diagnostic subtlety for whoever implements: `yt-dlp -F` (format listing) **succeeds** in this state even when the actual download fails; only a real fetch (e.g. `--download-sections "*0-8"`) proves downloadability.

**Why this matters:** the primary turnkey use case is *"paste the link right after the game."* That is exactly the window where the video sits in `post_live` (stream ended, VOD not yet processed) and where this failure mode lives.

**Recommended fix (two independent parts):**
1. **Plain-language `post_live` handling (the substantive fix).** Before download (or on `YTDLPError`), probe metadata (`--skip-download --print live_status` — one cheap subprocess, machinery mirrors `list_playlist_videos`). If `live_status` is `post_live`/`is_live`, replace the raw stderr with item-58-style guidance: *"This game just ended and YouTube is still processing the video. Wait ~an hour and paste the link again — or continue now at reduced quality (720p)."* The web app should render this as a friendly job status, not a failure wall. Fits the existing item-58 exception-triage seam.
2. **yt-dlp version bound (the guard-rail).** `yt-dlp>=2025.1` is unbounded, so fresh installs get whatever regression ships next. A hard upper pin is wrong for yt-dlp (it must track YouTube churn), but the known-good version should be recorded, and the `post_live` error path should mention the downloader version in its diagnostic (e.g. "downloader yt-dlp X.Y; 2025.10.14 is known to handle just-ended streams") so field reports are self-triaging. Optionally add a `<2026` cap with a comment as a stopgap until upstream fixes the SABR/`post_live` path — architect leans *against* the cap (it will silently rot) and *for* the recorded known-good + self-triaging message; implementer may counter-propose with data.

Design sizing: small (one probe helper + one error-mapping branch + tests with mocked runners for `post_live`/live/normal metadata; no pipeline changes).
